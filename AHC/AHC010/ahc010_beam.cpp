...
struct state {
    ...
    state() : i(0), j(0), d(0), gi(0), gj(0), gd(0), til(vector<vector<int>> (0)) {};
    state(int i_, int j_, int d_, int gi_, int gj_, int gd_, const vector<vector<int>>& t_) : i(i_), j(j_), d(d_), gi(gi_), gj(gj_), gd(gd_), til(t_) {};
    bool operator<(const state& s) const {
        return abs(i-gi) + abs(j-gj) < abs(s.i-s.gi) + abs(s.j-s.gj);
    }
};

vector<vector<int>> beam_search(int si, int sj, int sd, int gi, int gj, int gd, vector<vector<int>> t) {
    const int BEAM_SIZE = 200;
    vector<state> cands(1, state(si,sj,sd,gi,gj,gd,t));
    rep(_,400) {
        vector<state> ncands;
        for (state s : cands) {
            ...
            for (int x : can[nd]) {
                ...
                ncands.push_back(state(ni,nj,nd,gi,gj,gd,s.til));
...
vector<vector<int>> solve() {
    ...
    ans[29][29] = 4;
    ans = beam_search(29,29,0,0,29,3, ans);
    return ans;
}
