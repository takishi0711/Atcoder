...
string solve() {
    ...
    while (utility::mytm.elapsed() < TL) {
        uniform_int_distribution<> rands(0, ans.size());
        int idx = rands(mt);
        int co_i = -1;
        string co_ans = ans;
        double co_score = compute_E(ans);
        rep(i,4) {
            string nans = ans;
            nans.insert(nans.begin() + idx, dir[i]);
            double now_score = compute_E(nans);
            if (now_score > co_score) {
                co_score = now_score;
                co_i = i;
                co_ans = nans;
            }
