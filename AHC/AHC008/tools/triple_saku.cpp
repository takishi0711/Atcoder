...
vector<vector<P>> close_group(15);
...
void second_goal_init(int split, int odd) {
    if (split == 2) {
        ...
        rep(i,5) close_group[0+odd].push_back(P(14-i*3,9+odd*11));
        rep(i,5) close_group[2+odd].push_back(P(29-i*3,9+odd*11));
        ...
        rep(i,3) close_group[0+odd].push_back(P(8-i*3,9+odd*11));
        rep(i,3) close_group[2+odd].push_back(P(17-i*3,9+odd*11));
        rep(i,4) close_group[4+odd].push_back(P(29-i*3,9+odd*11));
        ...
        rep(i,2) close_group[0+odd].push_back(P(5-i*3,9+odd*11));
        rep(i,2) close_group[2+odd].push_back(P(11-i*3,9+odd*11));
        rep(i,3) close_group[4+odd].push_back(P(20-i*3,9+odd*11));
        rep(i,3) close_group[6+odd].push_back(P(29-i*3,9+odd*11));
        ...
        rep(i,2) close_group[0+odd].push_back(P(5-i*3,9+odd*11));
        rep(i,2) close_group[2+odd].push_back(P(11-i*3,9+odd*11));
        rep(i,2) close_group[4+odd].push_back(P(17-i*3,9+odd*11));
        rep(i,2) close_group[6+odd].push_back(P(23-i*3,9+odd*11));
        rep(i,2) close_group[8+odd].push_back(P(29-i*3,9+odd*11));
        order_three[0+odd] = {P(2,9+odd*11), P(5,9+odd*11)};
        order_three[2+odd] = {P(8,9+odd*11), P(11,9+odd*11)};
...
// 第 2 段階次の目的位置決定 2
void pos_nex2(int man) {
    int idx_man = order_man[man];
    int i = hx[idx_man], j = hy[idx_man];
    vector<pair<int, P>> vec;
    for (auto p : close_group[man]) {
        int ni = p.first, nj = p.second;
        if ((G1[ni][nj-1] != -1 && cnt_pet[ni/3 + 10*(man%2)]) || (G1[ni][nj+1] != -1 && cnt_pet[ni/3 + 10 + 10*(man%2)])) {
            vec.emplace_back(dist1(i,j,ni,nj), P(ni,nj));
        }
    }
    if (vec.size() == 0) {
        goal_close[man] = close_group[man][close_group[man].size()/2];
    } else {
        sort(vec.begin(), vec.end());
        goal_close[man] = vec[0].second;
    }
}
        ...
        if (prepar == false) {
            rep(man,m) {  
                ...
                if (prepar1_done[man] == false) { // 最初の定位置へ
                    ...
                    } else if (!dog_exist && prepar2_done[man] == false) { // いないなら第 2 段階の定位置へ
            ...
            vector<int> cnt_man(35);
            rep(man,m) {
                cnt_man[where_room(hx[man], hy[man])]++;
            }
            for (int man = m-1; man >= 0; man--) {
                int idx_man = order_man[man];
                int i = hx[idx_man], j = hy[idx_man];
                pos_nex2(man);
                bool ok = true;
                if (i%3 == 2) {
                    if (G1[i][j-1] != -1 && petok(i,j-1) && cnt_pet[i/3 + 10*(man%2)] && !cnt_man[i/3 + 10*(man%2)]) {
                        build(move_human, idx_man, 1);
                        ok = false;
                    } else if (G1[i][j+1] != -1 && petok(i,j+1) && cnt_pet[i/3 + 10 + 10*(man%2)] && !cnt_man[i/3 + 10 + 10*(man%2)]) {
                        build(move_human, idx_man, 3);
                        ok = false;
                    }
                }
                if (ok && P(i,j) != goal_close[man]) {
