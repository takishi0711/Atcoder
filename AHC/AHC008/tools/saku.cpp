        ...
        if (prepar1) {
            ...
            rep(man,m) {
                if (man < 5) {
                    ...
                    } else if (man != 2 && G1[i+1][j] != -1 && petok(i+1,j) && cnt_pet[(i+1)/3]) {
            ...
            bool close_ok = true;
            for (int idx : open_room) {
                if (idx == 5) continue;
                if (cnt_pet[idx]) close_ok = false;
            }
            if (close_ok) {
                close1 = false;
                // cout << "close1 done" << endl;
            }
        } else if (close_dog) {
            vector<int> cnt_pet(11);
            rep(pet,n) {
            ...
            // if (_) {
            //     cout << "#";
            //     for (int x : open_room) cout << x << " ";
            //     cout << endl;
            // }
            // cout << "#";
            rep(pet,n) {
                if (pt[pet] == 4) {
                    if (open_room.count(where_room(px[pet], py[pet]))) {
                        dog_out = true;
                        // cout << where_room(px[pet], py[pet]) << " ";
            ...
            if (dog_out) { // 犬いる
                ...
                rep(man,m) {
                    int i = hx[man], j = hy[man];
                    if (pre_nodog) {
                        move_human += f1(man, hx[man], hy[man], 15, left_col);
                        if (hx[man] == 15 && hy[man] == left_col) {
                            goal_man.insert(man);
                        }
                    } else {
                        if (G1[i+1][j] != -1 && petok(i+1,j) && cnt_pet[(i+1)/3]) {
            ...
            rep(man,m) {
                ...
                if (P(i,j) != close_point[man%5]) {
                    ...
                    if (G1[i-1][j] != -1 && petok(i-1,j) && cnt_pet[(i-1)/3]) {
                        build(move_human, man, 0);
                        open_room.erase((i-1)/3);
                    } else if (man%5 != 2 && G1[i+1][j] != -1 && petok(i+1,j) && cnt_pet[(i+1)/3]) {
                        build(move_human, man, 2);
                        open_room.erase((i+1)/3);
                    } else {
                        move_human += '.';
