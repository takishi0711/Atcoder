...
int main() {
    ...
    close_point[1] = P(21, 15);
    close_point[2] = P(13, 15);
    close_point[3] = P(5, 15);
    close_point[4] = P(1, 15);
    close_point[5] = P(9, 15);
    close_point[6] = P(17, 15);
    close_point[7] = P(25, 15);
    close_point[8] = P(1, 15);
    close_point[9] = P(9, 15);
    close_point_pair[close_point[0]] = close_point[7]; close_point_pair[close_point[7]] = close_point[0]; 
    close_point_pair[close_point[1]] = close_point[6]; close_point_pair[close_point[6]] = close_point[1]; 
    close_point_pair[close_point[2]] = close_point[5]; close_point_pair[close_point[5]] = close_point[2]; 
    close_point_pair[close_point[3]] = close_point[4]; close_point_pair[close_point[4]] = close_point[3]; 
    int pos = 0;
    if (dog_exist) {
        rep(i,m) {
            if (i == 3) continue;
    ...
    rep(_,300) {
        ...
        bool dog_in_room = true;
            rep(pet,n) {
                if (pt[pet] == 4) {
                    if (where_room(px[pet], py[pet]) != 15) dog_in_room = false;
                }
            }
            rep(man,m) {
                int i = hx[man], j = hy[man];
                if (man != 3 && G1[i][j+1] != -1 && petok(i,j+1) && dog_in_room) {
            ...
            vector<int> cnt_pet(20);
            rep(pet,n) {
                cnt_pet[where_room(px[pet], py[pet])]++;
            }
            vector<int> cnt_man(20);
            rep(man,m) {
            ...
            rep(man,m) {
                ...
                if (dog_exist && man == 3) {
                    ...
                    if (G1[i][j-1] != -1 && petok(i,j-1) && cnt_pet[i/4] && !cnt_man[i/4]) {
                        build(move_human, man, 1);
                    } else if (G1[i][j+1] != -1 && petok(i,j+1) && cnt_pet[i/4 + 8] && !cnt_man[i/4 + 8]) {
                        build(move_human, man, 3);
                    } else if (!close_point_exist[close_point_pair[goal_close[man]]]) {
                        move_human += '.';
